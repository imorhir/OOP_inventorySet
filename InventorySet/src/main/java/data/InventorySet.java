...
final class InventorySet implements Inventory {
  ...
  public Iterator<Record> iterator(Comparator<Record> comparator) {
	  List <Record> list = new  ArrayList<Record>(_data.values());
  ...
  Record checkOut(Video video) {
	  ...
	  if( e == null || e.numOut == e.numOwned)
	  {
		  throw new IllegalArgumentException();
	  }
	  RecordObj v = new RecordObj(video,e.numOwned,e.numOut,e.numRentals);
	  e.numOut +=1;
	  e.numRentals+=1;
	  return v;
   ...
   */
  Map clear() {
    HashMap<Video,Record> d = new HashMap<Video,Record>(_data);
    _data.clear();
    return d;
  }
